...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement pstmt=con.prepareStatement("select * from bossuser where mobile=? and pword=?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			...
			if(rs.next()) {
		        ...
		        out.println("</head>");
		        out.println("<body>");
		        out.println("<h1>Welcome, " + ud.getName().toUpperCase() +"!"+"</h1>");
		        out.println("</body></html>");
		        RequestDispatcher rd=request.getRequestDispatcher("ballot.html");
